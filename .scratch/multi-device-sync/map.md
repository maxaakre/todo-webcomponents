...
- [Migrate to the status enum (schema v1 → v2)](issues/13-status-enum-migration.md): **Built and shipped.** `done: boolean` is gone; `status` is live; **no hard delete remains in `src/`**. New pure `migrate.ts` module; `storage.ts` migrates on read and **leaves the v1 bytes on disk untouched until a save**, so the multi-tab write guard still compares against what is really there. `×` is relabelled **"Erase"** — the one deliberate user-visible change. **62 tests passing** (was 56), and the new ones were mutation-checked: five mutations, all caught. **Not hand-verified in a browser** — worth five minutes on the deployed URL once ticket 01 lands.
...
- **A Task that is visible nowhere. Severity raised by ticket 05.** *(2026-09-25: on one device this is now closed — the app shows a collapsed **Tomorrow** section, and rescheduling from Today is allowed. The two-device case below still stands: a Task dated tomorrow by a device in a later timezone can be two Days ahead on the other.)* A Task dated tomorrow is in neither Today nor Triage. One device hits this only by planning ahead; two devices hit it whenever they disagree about the date — a Task created just after midnight on the laptop is invisible on a phone in an earlier timezone. **Unlike every other divergence in this map, this one is silent.** It is the inherited "Upcoming view" question, and it is now the fog entry most likely to need a ticket of its own.
