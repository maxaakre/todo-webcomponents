...
- **`checked` reflects** for styling. So the attribute cannot also hold the *initial* state, like it does on a native checkbox. The initial state is captured once, on first connect, and restored on `form.reset()`.
